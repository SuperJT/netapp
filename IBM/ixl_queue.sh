        ...
        if (( ${CURRENT_VALUES[$key]} != 0 )); then
            ...
            if (( ${CURRENT_VALUES[$key]} <= ${PREV_VALUES[$key]:-0} )); then
                ...
                if (( ${COUNTERS[$key]} >= $THRESHOLD )); then
                    ngsh -c "set d -c off;system node autosupport invoke-diagnostic -node local -subsystems nic -message Likely-NIC-Hang-on-$key"
                    ## Trigger tape.diagMsg with current long_running operations
                    ngsh -c "set d -c off;event generate -node local -message-name tape.diagMsg Likely-NIC-Hang-on-$key"
