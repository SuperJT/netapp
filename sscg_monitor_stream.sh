...
FILER_LIF="10.30.253.60.111" #not used


## Client IP
CLIENT_IP='"147.141.74.2\|147.141.74.3"' #not used
...
compare_drops() {
	...
	if [ "$TESTING" != 1 ]; then
	    ...
	    ## Testing command
	    local NEW_CG_INFO=($(cat source/badstream/netstat_no_bad_stream.txt| grep "$INTERESTING_COUNTER"|awk '{print $'"$COUNTER_POSITION"'}'| sed 's/[^0-9]*//g'))
	fi
...
zero_streams() {
	...
	if [ "$TESTING" != 1 ]; then
	    ## Production command
	    declare RESULT=($(netstat -anCETc| grep "147.141.74.2\.\|147.141.74.3\."| grep tcp4))
	else
	    ## Testing command
	    declare RESULT=($(cat source/badstream/netstat_no_bad_stream.txt| grep "147.141.74.2\.\|147.141.74.3\." | grep tcp4))
	fi
...
if [ "$LOOP" != "1" ]; then
    if [ "$TESTING" != 1 ]; then
	    ## Production command
		pkill tcpdump
	    ngsh -c "autosupport invoke -node * -message $MESSAGE -type test"
		sleep $SLEEPTIME
